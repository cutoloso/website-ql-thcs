...
<div class="col-md-10 offset-1 card" ng-controller="ProfileController" style="max-width: 100%;">
	<div class="row">
		...
		<div class="col-md-8 profile" >
			<h1>{{$profile->qt_hoTen}}</h1>
			<p class="title">Thông tin cá nhân</p>
			<p>Ngày sinh: {{$profile->qt_ngaySinh}}</p>
			<p>Phái: {{$profile->qt_phai != 0 ? 'Nam':'Nữ'}} </p>
			<p>Số điện thoại: 0234567889</p>
			<p>Địa chỉ: {{$profile->qt_diaChi}}</p>
			<p>Email: {{$profile->qt_email}}</p>
			<p>Chức vụ: Quản trị</p>
		</div>
	</div>
</div>
